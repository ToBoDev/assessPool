...
postFST <- function(filetype, project_name, fst.calc, as, popcomb, strong_diff, p_cutoff, fasta_generation, ref_file, first_mincov, last_mincov, cov_step){
  ...
  file.copy(from=ref_file, to=paste(working_dir,project_name,ref_file, sep="/"), overwrite = TRUE) 
    ...
    #import FST output, save as dataframes
    for(ft in filetype){
      for(i in 1:C){
    ...
    write.csv(popl.mast.export, paste(working_dir,"/", project_name, "/output/", fst.calc, "/", project_name,"_allvar_postPopoolation2.csv", sep=""), row.names=FALSE)
    m <- paste("Exported all variable sites to ","output/", fst.calc, "/", project_name,"_allvar_postPopoolation2.csv", sep="")
    message(m); write.log(m, paste(working_dir, project_name, "logs/analysis.log", sep="/"))
    ...
    write.csv(popl.mast.export, paste(working_dir,"/", project_name,"/output/", fst.calc, "/", project_name, "_calledAllPools_postPopoolation2.csv", sep=""), row.names=FALSE)
    m <- paste("Exported variable sites called in all pools to ","output/", fst.calc, "/", project_name,"_calledAllPools_postPopoolation2.csv", sep="")
    message(m); write.log(m, paste(working_dir, project_name, "logs/analysis.log", sep="/"))
    ...
    if (".fst" %in% filetype){
      ...
      write.csv(popl.appfx.export, paste(working_dir,"/", project_name,"/output/", fst.calc, "/", project_name, "_strongly_differentiated_sites.csv", sep=""), row.names=FALSE)
      m <- paste("Exported strongly differentiated sites to ","output/", fst.calc, "/", project_name,"_strongly_differentiated_sites.csv", sep="")
      message(m); write.log(m, paste(working_dir, project_name, "logs/analysis.log", sep="/"))
      ...
      write.csv(popl.fixed.export, paste(working_dir,"/", project_name,"/output/", fst.calc, "/", project_name, "_fixed_snps.csv", sep=""), row.names=FALSE)
      m <- paste("Exported alternatively fixed sites to ","output/", fst.calc, "/", project_name,"_fixed_snps.csv", sep="")
      message(m); write.log(m, paste(working_dir, project_name, "logs/analysis.log", sep="/"))
      ...
      if (fasta_generation==TRUE){
        ...
        write.fasta(sequences=as.list(sd_fasta$Seq), names=sd_fasta$CHROM, file.out=paste(working_dir,"/", project_name,"/output/", fst.calc, "/", project_name, "_strongly_differentiated_sites_fasta.fa", sep=""), nbchar=1000)
        m <- paste("Wrote ", nrow(sd_fasta), " contigs to /output/",fst.calc, "/", project_name, "_strongly_differentiated_sites_fasta.fa", sep="")
        message(m); write.log(m, paste(working_dir, project_name, "logs/analysis.log", sep="/"))
        
        #generate FASTA from alternatively fixed sites
        alt_fasta <- getFasta(popl.fixed, ref_file_df)
        write.fasta(sequences=as.list(alt_fasta$Seq), names=alt_fasta$CHROM, file.out=paste(working_dir,"/", project_name,"/output/", fst.calc, "/", project_name, "_fixed_sites_fasta.fa", sep=""), nbchar=1000)
        m <- paste("Wrote ", nrow(alt_fasta), " contigs to /output/", fst.calc, "/", project_name, "_fixed_sites_fasta.fa", sep="")
        message(m); write.log(m, paste(working_dir, project_name, "logs/analysis.log", sep="/"))
      }
    }
