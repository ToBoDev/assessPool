...
echo "It reports a low range, based on a 50% binomial probability of observing the second allele in a heterozygote, and a high range based on a 25% probability."
echo "NOTE: these values may be slightly overestimated for pooled data."
echo ""
...
echo ""
echo $IND "pools and" $LOCI "SNPs equals" $GENO "total genotypes"
GENO=$(( $GENO - $MISSING))
...
echo ""
echo "Scorched earth scenario: What if all low depth homozygote genotypes are errors?"
