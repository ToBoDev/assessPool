...
#Steps - T to run listed step, F to skip
filtering=T
calculate_FST=T
visualizations=T
...
#list current sample names from VCF
samples(scanVcfHeader(vcf_file))

#IMPORTANT: create a file <project_name>_fixed_samples.txt to rename your vcf samples. List each sample in the SAME order as the output from the line above, with whatever desired name changes you wish to make

#`bcftools reheader -s <txt file of samples> -o <out.vcf> -i <in.vcf>`
...
min_cov <- 5 #minimum coverage (in ALL populations) (DEFAULT=2)
...
pool_size <- c(25,23,39,39,23,31,25,32,42,25,37,25,25,25,29,23) #vector of all pool sizes in same order as POPS (one value if all pools are the same size)
POPS
...
#manually reorder populations for plotting
# pool_order <- c("LAY",
#                 "GAR",
#                 "FFS",
#                 "OAHU-NS",
...
#indlude to force diagonal on matrix upper-left -> lower-right
...
plot_heatmaps(fst.c="popoolation", include_called_allpops=F)

if(include_called_allpops){
  plot_heatmaps(fst.c="popoolation", include_called_allpops=T)
}

plot_heatmaps(fst.c="poolfstat", include_called_allpops=F)

if(include_called_allpops){
  plot_heatmaps(fst.c="poolfstat", include_called_allpops=T)
  }
