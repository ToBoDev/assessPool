...
#initialize vcftools path - do not edit here
vcftools_PATH <- system("which vcftools", intern = T)
...
#edit this path and re-run chunk if warned below
vcftools_PATH.mod <- "/path/to/vcftools0.1.12_or_higher/"
#example: vcftools_PATH <- "/home/evan/miniconda2/bin/"

vcftools_path_export <- paste("export PATH=", vcftools_PATH, "/:$PATH;", sep = "")

version <-system(paste(vcftools_path_export,'vcftools'), intern=T)[2]

if(grepl("0.1.11", version)) {
     assign("vcftools_path_export",vcftools_PATH.mod)
     stop("Please update vcftools to v0.1.12 or later, and/or edit vcftools_PATH above")
   } else {
